...
def smooth_cosine_similarity(m, v):
    ...
    m_norm = tf.sqrt(tf.reduce_sum(tf.pow(m, 2),2))         #b*N
    v_norm = tf.sqrt(tf.reduce_sum(tf.pow(v, 2),1))         #b
    m_norm=tf.reshape(m_norm,[shape_x[0], shape_x[1], 1])   #b*N*1
    v_norm = tf.reshape(v_norm, [shape_y[0], 1, 1])            #b*1
    m_dot_v = tf.matmul(m, tf.reshape(v, [shape_y[0],-1, 1]))     #b*N*1
    m_v_norm=tf.matmul(m_norm ,v_norm)+1e-3                         #b*N*1
    similarity = tf.div(tf.reshape(m_dot_v, [shape_y[0],-1]), tf.reshape(m_v_norm, [shape_y[0],-1]))       #b*N
    return similarity
...
def circular_convolution(v, k):
    ...
    kernels = []
    for i in range(size):
        indices = [loop(i+j) for j in range(kernel_shift, -kernel_shift-1, -1)]        #3
        v_=[]
        for z in range(int(v.get_shape()[0])):
            S = tf.gather(v[z], indices)
            v_.append(S)
        #v_=tf.stack(v_)                                                   #b*3
        #v_ = tf.gather(v, indices)
        kernels.append(tf.reduce_sum(v_ * k, 1))                                #N*b
    #kernels=[tf.transpose(kernels)]                                               #N*b
    return tf.dynamic_stitch([i for i in xrange(size)], kernels)                #N*b
...
def outer_product(*inputs):
    ...
    for idx, input_ in enumerate(inputs):
        if len(input_.get_shape()) == 2:
            inputs[idx] = tf.reshape(input_, [input_.get_shape()[0],input_.get_shape()[1], 1] if idx % 2 == 0 else [input_.get_shape()[0], 1, input_.get_shape()[1]])
